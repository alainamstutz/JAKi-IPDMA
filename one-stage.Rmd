...
df_long <- df_long %>% # create censoring indicator
  mutate(censor = case_when(is.na(mort_60_n) ~ 1,
                            TRUE ~ 0)) 
...
# df_long %>%
#   dplyr::select(id_pat, trial, trt, time, death_time, death_time_60, mort_28, mort_60, mort_60_n, censor, death_reached, discharge_reached, discharge_time, sex, clinstatus_baseline) %>%
#   # filter(id_pat == "80804") %>%
#   # filter(id_pat == "80504") %>%
#   # filter(mort_60 == 1) %>%
...
risk.boot.1 <- function(data, indices) {
  ...
  ### Fit pooled logistic model to estimate discrete hazards
  pool.boot <- speedglm(formula = mort_60_n==1 ~ trt + time + timesqr + age + ...
                                    I(trt*time) +
                                    I(trt*timesqr),
                family = binomial(link = 'logit'),
                data = d,
...
intersect_y <- 0.112 # Example y-coordinate 10.8, but CAVE: RUXCOVID is out
intersect_y2 <- 0.132 # Example y-coordinate 12.9,but CAVE: RUXCOVID is out
