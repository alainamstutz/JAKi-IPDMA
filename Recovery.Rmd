...
df_in %>% # respiratory support at baseline has various options per patient!
...
unique(df_baseline$INDECOD)
unique(df_baseline$INTRT)
unique(df_baseline$EPOCH)
unique(df_baseline$INEVINTX)
unique(df_baseline$VISIT)
...
table(df$clinstatus_baseline_all, useNA = "always")
...
```
Discussion points:
* time to death -> DMDY in dm set? But includes event dates after day 28? Censoring info?
* (time to) withdrawal? (time to) LTFU? (time to) discharge?
* time to mechanical ventilation? Only event y/n "DURING 28 DAYS AFTER FIRST RANDOMIZATION" ?!
* Several baseline severity/respiratory support per patient?
* Where is Symptom duration?

* What was the maximum systematic follow-up of all study participants? According to protocol: 6months
* Deaths after discharge systematically documented?
* Re-admissions systematically documented? Probably not.
* Any transfer to hospice occurred? Probably yes.

*** USE INOCCUR for the baseline severity!!!
