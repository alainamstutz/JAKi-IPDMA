...
forest.meta(age.mort28,
            ...
            xlab.pos = 0.0,
            fs.xlab = 11,
            digits = 3  # Set decimal places to 4
            )

# Open a pdf file
pdf("./fp_interactions_age.pdf", width=10, height=5)
forest.meta(age.mort28,
            hetstat = F,
            leftcols = c("studlab"),
            leftlabs = c("Trial"),
...
# Open a pdf file
pdf("./fp_sg_age_red.pdf", width=10, height=10)

mort28.age.red <- metagen(TE = log(hazard_odds_ratio),
                      seTE = standard_error,
                      studlab = variable,
                      data = df_sg_age_mort28,
                      #n.e = n_intervention_tot,
...
summary(mort28.age.red)
forest.meta(mort28.age.red,
            rightcols = c("effect"),
            rightlabs = c("aOR"),
            leftcols = c("studlab"),
            leftlabs = c("Trial"),
...
# pdf(file = "Fig3_main_foresplot.pdf",
#     width = 15, 
#     height = 15)
...
# Forestplot subgroup - on primary endpoint - for POSTER
```{r, fig.width=10, fig.height=10}
# ...and ICEMAN assessments
iceman <- c("NA", "", "", "", "moderate\ncredibility", "", "NA", "", "", "", "NA", "", "", "", "NA", "", "", "NA", "")

pdf(file = "Fig3_main_foresplot_poster.pdf",
    width = 9,
    height = 10)
...
mort28_fp %>%
  forestplot(labeltext = c(subgroup, p_int, iceman),
             txt_gp = fpTxtGp(label = gpar(fontfamily = font, cex=1),
                              ticks = gpar(cex=0.88),
                              summary = gpar(cex=1),
                              xlab = gpar(cex=0.88)),
...
proportions <- df_aesi_tot %>%
  ...
  mutate(denom = case_when(Group == "No JAK inhibitor" ~ 6255, #### ADAPT if NEW TRIALS, see "Collect all treatment effect estimates" - BUT run descriptive first to have updated df_tot !!!
                           Group == "JAK inhibitor" ~ 6647)) %>%   #### ADAPT if NEW TRIALS, see "Collect all treatment effect estimates" - BUT run descriptive first to have updated df_tot !!!
