...
df_comorb <- read_xlsx("/Users/amstutzal/Library/CloudStorage/OneDrive-usb.ch/Dokumente - JAKi IPDMA data source management/General/COVINIB/COVINIB - CKD & autoimmune disease.xlsx")
```
...
trt_coef <- coef(mort.28.age)[grep("^trt:", names(coef(mort.28.age)))]
log_odds_ratio <- exp(trt_coef)
ci <- exp(confint(mort.28.age)[grep("^trt:", names(coef(mort.28.age))), ])
se <- summary(mort.28.age)$coefficients[grep("^trt:", names(coef(mort.28.age))), "Std. Error"]
p_value <- summary(mort.28.age)$coefficients[grep("^trt:", names(coef(mort.28.age))), "Pr(>|z|)"]
...
trt_coef <- coef(mort.28.age.firth)[grep("^trt:", names(coef(mort.28.age.firth)))]
log_odds_ratio <- exp(trt_coef)
ci <- exp(confint(mort.28.age.firth)[grep("^trt:", names(coef(mort.28.age.firth))), ])
se <- sqrt(diag(vcov(mort.28.age.firth)))[grep("^trt:", names(coef(mort.28.age.firth)))]
p_value <- mort.28.age.firth$prob[grep("^trt:", names(coef(mort.28.age.firth)))]
print(log_odds_ratio)
print(ci)
print(p_value)
...
df <- df %>% 
  mutate(age_70 = case_when(age < 70 ~ 0,
                            age > 69 ~ 1))
df <- df %>% 
  mutate(age_70 = case_when(age < 60 ~ 0,
                            age > 59 ~ 1))
# table(df$age_70, useNA = "always")
table(df$age_70, df$mort_28, useNA = "always")
table(df$mort_28, df$trt, df$age_70, useNA = "always") # only two events in control in the young ones.. => in subgroup of young ones, point estimate should be on side of JAKi. BUT, RANDOM CUT-OFF!!! If you set cut-off to 60 years, then the two events are in the control group of the OLD ones and the point estimates reverses of course for old/young! However, the p-int. using linear age linear is fine.
...
trt_coef <- coef(mort.28.age.a70.firth)[grep("^trt", names(coef(mort.28.age.a70.firth)))]
hazard_odds_ratio <- exp(trt_coef)
ci <- exp(confint(mort.28.age.a70.firth)[grep("^trt", names(coef(mort.28.age.a70.firth))), ])
se <- sqrt(diag(vcov(mort.28.age.a70.firth)))[grep("^trt", names(coef(mort.28.age.a70.firth)))]
p_value <- mort.28.age.a70.firth$prob[grep("^trt", names(coef(mort.28.age.a70.firth)))]
print(hazard_odds_ratio)
print(ci)
print(p_value)
...
trt_coef <- coef(mort.28.age.a70)["trt"]
hazard_odds_ratio <- exp(trt_coef)
ci <- exp(confint(mort.28.age.a70)["trt", ])
se <- summary(mort.28.age.a70)$coefficients["trt", "Std. Error"]
# p_value <- summary(mort.28.age.a70)$coefficients["trt", "Pr(>|z|)"]
# print(hazard_odds_ratio)
# print(ci)
# print(p_value)
...
trt_coef <- coef(mort.28.age.b70.firth)[grep("^trt", names(coef(mort.28.age.b70.firth)))]
hazard_odds_ratio <- exp(trt_coef)
ci <- exp(confint(mort.28.age.b70.firth)[grep("^trt", names(coef(mort.28.age.b70.firth))), ])
se <- sqrt(diag(vcov(mort.28.age.b70.firth)))[grep("^trt", names(coef(mort.28.age.b70.firth)))]
p_value <- mort.28.age.b70.firth$prob[grep("^trt", names(coef(mort.28.age.b70.firth)))]
print(hazard_odds_ratio)
print(ci)
print(p_value)
...
trt_coef <- coef(mort.28.age.b70)["trt"]
hazard_odds_ratio <- exp(trt_coef)
# ci <- exp(confint(mort.28.age.b70)["trt", ])
# se <- summary(mort.28.age.b70)$coefficients["trt", "Std. Error"]
# p_value <- summary(mort.28.age.b70)$coefficients["trt", "Pr(>|z|)"]
# print(hazard_odds_ratio)
# print(ci)
# print(p_value)
