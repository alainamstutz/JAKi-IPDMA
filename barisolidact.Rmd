...
# results publication used mITT set
# df <- df %>% 
#   filter(imp_adm_yn == 1)

```
Questions: 
1) CAVE: use ITT set - see ACTT-2
...
df <- df %>% 
  mutate(any_comorb = case_when(comorb_lung == 1 | comorb_liver == 1 | comorb_cvd == 1 |
                                ...
                                ~ 0))
table(df$any_comorb, useNA = "always")
...
table(df$comorb_cat, useNA = "always") # no missing
...
- mort_28 & mort_60 outcomes: 5 missing -> MICE as SENS
- new_mv_28 & new_mvd_28: 4 missing -> MICE as SENS
...
```
Discussion points
1) Standardize length of max follow-up across trials?
...
result_list[[10]] <- extract_trt_results(vir.clear.5, "viral clearance until day 5")
result_list[[11]] <- extract_trt_results(vir.clear.10, "viral clearance until day 10")
result_list[[12]] <- extract_trt_results(vir.clear.15, "viral clearance until day 15")
...
```
Discussion points
1) Make Fine-Gray for time to discharge the primary effect estimate!
